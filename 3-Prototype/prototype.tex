...
In deliverable D2.1 and D3.1 the current architecture has been written down using the AS(I)M approach and as mentioned before a working client-server network based on message-passing has been developed. That classic model was used to derive a new payment network based on block-chain approach.

The idea for the new network comprise of using a publicly maintained and easy to use block-chain which supports the though of a mutual interest-free credit system enabling account based balances rather than token based currencies only. This thought led to a prototypical implementation using hyperledger fabric together with hyperledger composer.

Regardless of the choice of hyperledger environment it also has been important having central payment network which is reliable, verifiable but also to a specific extend controllable in order to impose the basic Sardex payment rules on it.
...
\begin{figure}[htbp]
  ...
  \caption{\bf\small TODO: Network Structure implemented by the Prototype}
